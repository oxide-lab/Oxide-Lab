...
impl QuantizedAttention {
    ...
    pub fn forward(
        ...
        let q = if let Some(attention_bq) = &self.attention_bq {
            q.broadcast_add(attention_bq)?
        } else {
            q
        };

        let k = if let Some(attention_bk) = &self.attention_bk {
            k.broadcast_add(attention_bk)?
        } else {
            k
        };

        let v = if let Some(attention_bv) = &self.attention_bv {
            v.broadcast_add(attention_bv)?
        } else {
            v
        };
...
impl GGUFQwen2Moe {
    ...
    pub fn clear_kv_cache(&mut self) {
        ...
        // Force CUDA/Metal synchronization to ensure clean state

...
pub async fn is_cuda_installed(
    ...
    // Old location (used by older builds): app_data_folder_path/llamacpp/lib/libname
    let old_path = std::path::PathBuf::from(&app_data_folder_path)
        .join("llamacpp")
        .join("lib")
        .join(libname);
...
mod tests {
    ...
    async fn test_is_cuda_installed_migration() {
        ...
        // Setup old path: app_data/llamacpp/lib/libcudart.so.12
        let old_lib_dir = app_data_dir.path().join("llamacpp").join("lib");
        ...
        let installed = is_cuda_installed(
            ...
            version.to_string(),
            os_type.to_string(),
            app_data_dir.path().to_string_lossy().to_string(),
        )
        .await
        .unwrap();
    ...
    async fn test_is_cuda_installed_already_exists() {
        let backend_dir = tempfile::tempdir().unwrap();
        let app_data_dir = tempfile::tempdir().unwrap(); // Empty
